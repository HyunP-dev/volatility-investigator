...
VOL_PATH = os.path.expanduser("~\\Desktop\\교육 사용 도구\\volatility3-2.4.1\\volatility3-2.4.1")
...
from volatility3.framework import contexts, plugins, automagic, interfaces
from volatility3.framework.constants import ProgressCallback
from volatility3.framework.renderers import UnreadableValue, NotApplicableValue
...
class MainModel:
    ...
    @staticmethod
    def construct_runner(plugin_name: str, image_path: str, progress_callback: ProgressCallback):
        ctx = contexts.Context()
        automagics = automagic.available(ctx)
        plugin_list = framework.list_plugins()
        plugin = plugin_list[plugin_name]
        base_config_path = "plugins"
        ctx.config["automagic.LayerStacker.single_location"] = volatility3.cli.CommandLine.location_from_file(image_path)
    ...
    def run_ps_list(self, progress_callback: ProgressCallback):
        ...
        rows = []
        for _, e in MainModel.construct_runner(plugin_name, 
                                               self._image_path, 
                                               progress_callback)._generator():
